/*
  ...
  1. Cambios en la tabla customers
    - Agregar `nombre` (text) - Nombre(s) del cliente
    - Agregar `apellido_paterno` (text) - Apellido paterno
    - Agregar `apellido_materno` (text, opcional) - Apellido materno
    - Mantener `nombre_completo` como campo calculado para compatibilidad
    - Agregar índices para búsqueda eficiente por todos los campos

  2. Índices adicionales
    - Índice en email para búsqueda rápida
    - Índice en nombre, apellido_paterno, apellido_materno
    - Índice de texto completo para búsqueda general

  3. Notas importantes
    - Los campos existentes se mantienen para compatibilidad
    - Se agregan triggers para mantener nombre_completo actualizado
*/
